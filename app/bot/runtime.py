...
async def menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
...
async def order_service(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    db = SessionLocal()
    try:
        services = db.query(ServiceCatalog).filter(ServiceCatalog.platform.ilike(platform), ServiceCatalog.enabled.is_(True)).limit(10).all()
    finally:
        db.close()
    if not services:
    ...
    await update.message.reply_text("Select service ID and send it:\n" + "\n".join([f"{s.id}: {s.service_name} (₹{s.base_rate})" for s in services]))
    return ORDER_LINK
...
async def order_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("order", {}).get("service_id") is None:
        if not text.isdigit():
            ...
            return ORDER_LINK
        context.user_data["order"]["service_id"] = int(text)
...
async def order_qty(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(f"Order captured ✅\nPlatform: {data.get('platform')}\nService ID: {data.get('service_id')}\nLink: {data.get('link')}\nQty: {q}")
...
async def daily_reward(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await target(f"Daily reward: {status}")


async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    uid = update.effective_user.id
    target = update.callback_query.message.reply_text if update.callback_query else update.message.reply_text
    await target(f"Referral link format: /refer {uid}")
...
async def refer_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    db = SessionLocal()
    try:
        status = register_referral(db, int(context.args[0]), update.effective_user.id)
    finally:
        db.close()
...
def build_application() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(menu_click, pattern=r"^menu_"))
